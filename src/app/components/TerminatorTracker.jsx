...
export default function TerminatorTracker({ userLocation }) {
    ...
    const terminatorImages = [
        '/terminator-t800.png',
        '/terminator-t1000.png', 
        '/terminator-tx.png',
        '/terminator-t3000.png',
        '/terminator-rev9.png'
    ];
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setTerminators(prev => prev.map(unit => ({
                ...unit,
            ...
            {terminators.map((unit) => (
                <div key={unit.id} className="bg-gray-800/50 border border-red-500/30 rounded p-3 relative overflow-hidden">
                    ...
                    <div className="flex items-center space-x-3">
                        {/* Imagen del Terminator */}
                        <div className="flex-shrink-0 w-12 h-12 bg-red-900/30 rounded border border-red-500/50 flex items-center justify-center relative overflow-hidden">
                            <div className="absolute inset-0 bg-gradient-to-br from-red-600/20 to-transparent"></div>
                            <div className="text-red-400 text-xs font-mono font-bold z-10">{unit.model}</div>
                            {/* Efecto de HUD */}
                            <div className="absolute top-1 left-1 w-2 h-2 border-l border-t border-red-500 opacity-60"></div>
                            <div className="absolute top-1 right-1 w-2 h-2 border-r border-t border-red-500 opacity-60"></div>
                            <div className="absolute bottom-1 left-1 w-2 h-2 border-l border-b border-red-500 opacity-60"></div>
                            <div className="absolute bottom-1 right-1 w-2 h-2 border-r border-b border-red-500 opacity-60"></div>
                        </div>
                        ...
                        <div className="flex-grow">
                            <div className="flex justify-between items-center">
                                ...
                                <div className={`text-xs font-bold ${getStatusColor(unit.status)} animate-pulse`}>
                                    ● {translate(unit.status, 'terminatorStatus', language)}
                                </div>
                            </div>
                                        ...
                                        <div className="w-16 h-1 bg-gray-700 rounded overflow-hidden">
                                            <div 
                                                className={`h-full transition-all duration-1000 ${
                                                    unit.batteryLevel > 70 ? 'bg-green-500' :
                                                    unit.batteryLevel > 30 ? 'bg-yellow-500' : 'bg-red-500'
                                                }`}
